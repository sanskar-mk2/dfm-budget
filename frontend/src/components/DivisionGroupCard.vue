...
const emit = defineEmits(['save-ratios'])
...
watch(() => props.group, (newGroup) => {
    if (newGroup && newGroup.divisions) {
        localDivisions.value = newGroup.divisions.map(div => ({ ...div }))
    }
}, { immediate: true })

// Computed properties
const totalRatio = computed(() => {
    return localDivisions.value.reduce((sum, div) => sum + div.effective_ratio, 0)
})
...
const hasChanges = computed(() => {
    return localDivisions.value.some(div => 
...
const normalizeRatios = (changedIndex, newRatio) => {
    ...
    const otherUnlocked = unlockedDivisions.filter(div => div.index !== changedIndex)
    const otherUnlockedSum = otherUnlocked.reduce((sum, div) => sum + div.effective_ratio, 0)
    ...
    if (otherUnlockedSum > 0) {
        ...
        otherUnlocked.forEach(div => {
            localDivisions.value[div.index].effective_ratio = div.effective_ratio * scaleFactor
        })
    }
}
...
const handleLockToggle = (divisionIndex) => {
    localDivisions.value[divisionIndex].locked = !localDivisions.value[divisionIndex].locked
}
...
const handleReset = () => {
    // Reset all divisions to their original ratios
    localDivisions.value = props.group.divisions.map(div => ({
        ...div,
        effective_ratio: div.division_ratio_2025,
        locked: false
    }))
}
</script>
...
<template>
    <div class="card bg-base-100 shadow-xl border border-base-300">
        <div class="card-body">
            ...
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h3 class="card-title text-lg">
                        {{ group.customer_class }} › {{ group.salesperson_name }} › {{ group.group_key }}
                    </h3>
                ...
                <div class="flex items-center gap-2">
                    <span :class="['text-sm font-mono', ratioStatus.class]">
                        Total: {{ (totalRatio * 100).toFixed(1) }}% {{ ratioStatus.icon }}
                    </span>
                </div>
            </div>
            ...
            <!-- Status Messages -->
            <div v-if="ratioStatus.status !== 'perfect'" class="alert alert-warning mt-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
                </svg>
                <span>
                    <span v-if="ratioStatus.status === 'over'">
                        Total ratio exceeds 100%. Adjust sliders to balance the ratios.
                    </span>
                    <span v-else>
